  ...
  return (
    <div className="py-20 px-6 sm:px-12 space-y-24">
      <div className="max-w-7xl mx-auto">
        ...
        {work.length === 0 ? (
          <div className="text-center py-16 text-zinc-500 text-sm font-light">No work experience listed yet.</div>
        ) : (
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="space-y-6">
            {work.map((item) => (
              <motion.div key={item.id} variants={fadeInUp} className="p-8 rounded-xl border border-zinc-800/80 bg-zinc-950/40">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="space-y-4">
            ...
            {education.length === 0 ? (
              ...
              education.map((edu) => (
                <motion.div key={edu.id} variants={fadeInUp} className="p-8 rounded-xl border border-zinc-800/80 bg-zinc-950/40">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="space-y-4">
            ...
            {certifications.length === 0 ? (
              ...
              certifications.map((cert) => (
                <motion.div key={cert.id} variants={fadeInUp} className="p-6 rounded-xl border border-zinc-800/80 bg-zinc-950/40">
